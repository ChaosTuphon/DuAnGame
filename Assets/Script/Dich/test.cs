...
public class test : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (inRange)
        {
            hit = Physics2D.Raycast(raycast.position, transform.right * transform.localScale.x, raycastLength, raycastMask);
            RayCastDebugger();
    ...
    void RayCastDebugger()
    {
        if (distance > attackDistance)
        {
            Debug.DrawRay(raycast.position, transform.right * raycastLength * transform.localScale.x, Color.red);

        }
        else if (attackDistance > distance)
        {
            Debug.DrawRay(raycast.position, transform.right * raycastLength * transform.localScale.x, Color.green);
        }
        Flip();
    }
